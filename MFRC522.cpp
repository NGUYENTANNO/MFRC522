...
void MFRC522::begin() 
{
  SPI.begin();          //Khoi tao module RC522
  digitalWrite(_sad, HIGH);
...
/******************************************************************************/
int MFRC522::ghi_the(byte block,byte *keyA,byte *data_write) 
{
   int trang_thai;  byte data[16];                 
    trang_thai = requestTag(MF1_REQIDL, data);                      //Bước 1: Dò tìm thẻ trên module
    if(trang_thai==0) //có thẻ trên module
    {
      trang_thai= antiCollision(data);                             //Bước 2: Kiểm tra nhiều thẻ      
      byte ten_the[5]; memcpy(ten_the,data,5);
      selectTag(ten_the);                                          //Bước 3: chỉ ta thẻ muốn truy xuất
      trang_thai = authenticate(MF1_AUTHENT1A,block,keyA,ten_the); //Bước 4: truy nhập mật khẩu thẻ  
      if(trang_thai == 0)                                              //Đăng nhập thành công 
       {
         trang_thai = writeToTag(block,data_write);                //Bước 5: đọc giá trị thẻ 
         if(trang_thai == 0)
         {
            haltTag(); 
           return 1; 
...
/************************************************************************************/
int MFRC522::doc_the(byte block,byte *keyA,byte *ket_qua)
{ 
    int trang_thai;  byte data[16];                                 //Bước 1: Dò tìm thẻ trên module
    trang_thai = requestTag(MF1_REQIDL, data);  
    if(trang_thai==0)                                               //có thẻ trên module
    {
      trang_thai= antiCollision(data);                          //Bước 2: Kiểm tra nhiều thẻ      
      byte ten_the[5];
      memcpy(ten_the,data,5);
      selectTag(ten_the);                                       //Bước 3: chỉ ta thẻ muốn truy xuất  
      trang_thai = authenticate(MF1_AUTHENT1A,block,keyA,ten_the);  //Bước 4: truy nhập mật khẩu thẻ  
      if(trang_thai == 0)                                               //Dang nhap thanh cong 
       {
         trang_thai = readFromTag(block,data);                      //Bước 5: đọc giá trị thẻ 
         byte i;
         for(i=0;i<16;i++)
         {
          ket_qua[i] = data[i];
